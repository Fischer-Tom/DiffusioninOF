...
float edgeStrength(CTensor<float>& aImage) {
    ...
    for (int i = 0; i < mag.size(); i++) {
        ...
        avg += mag.data()[i];
        if (mag.data()[i] > max) max = mag.data()[i];
    }
    avg /= mag.size();
...
void inpaint(CTensor<float> aMatching, CTensor<float> confidence, int time){
    int aXSize = aMatching.xSize();
    int aYSize = aMatching.ySize();

    CTensor<float> result;
    result.setSize(aMatching.xSize(), aMatching.ySize(),2);
    long i,j,k,t; // loop variables
    float tau = 0.25; // timestep
    float temp;
    float c;
    for (t=1;t<time;t++){
        for (i=1; i<aXSize-1; i++)
            for (j=1;j<aYSize-1; j++)
                for (k=0;k<2;k++) {
                    result(i,j,k) = aMatching(i,j, k);
                }
            ...
            for (j=1;j<aYSize-1; j++)
                for (k=0;k<2;k++){
                    if (confidence(i,j,0)==0)
                        aMatching(i,j,k) = (1-2*tau-2*tau)*result(i,j,k)
                                        + tau*result(i-1,j,k)
                                        + tau*result(i+1,j,k)
                                        + tau*result(i,j-1,k)
                                        + tau*result(i,j+1,k);
    ...
    CTensor<float> aMatchingImage(aMatching.xSize(),aMatching.ySize(),3);
    COpticFlow::flowToImage(aMatching,aMatchingImage,8);
    aMatchingImage.writeToPPM("Inpainted.ppm");
    COpticFlow::writeMiddlebury(aMatching, "Inpainted.flo");
}

void runFlow(CTensor<float>& aImage1, CTensor<float>& aImage2, CTensor<float>& aResult, float sigma,
             float alpha, float beta, float gamma, int t) {
    int aXSize = aImage1.xSize();
    int aYSize = aImage1.ySize();
    ...
    for (int ay = 4; ay < aYSize-4; ay+=aStep)
        for (int ax = 4; ax < aXSize-4; ax+=aStep) {
            if (aCornerness(ax,ay) < 0.125*aAverage) continue;
            ...
            int x1 = ax-r; if (x1 < 4) x1 = 4;
            int y1 = ay-r; if (y1 < 4) y1 = 4;
            int x2 = ax+r; if (x2 >= aXSize-4) x2 = aXSize-5;
            int y2 = ay+r; if (y2 >= aYSize-4) y2 = aYSize-5;
            ...
            for (int by = y1; by <= y2; by++)
                for (int bx = x1; bx <= x2; bx++) {
                    if (aEnergy2(bx,by) > 2.0f*aEnergy1(ax,ay) || aEnergy1(ax,ay) > 2.0f*aEnergy2(bx,by)) continue;
                    ...
                    if (dist < bestDist) {
                        best2Dist = bestDist; best2X = bestX; best2Y = bestY;
                        bestDist = dist; bestX = bx; bestY = by;
                    }
                    else if (dist < best2Dist) {
                        best2Dist = dist; best2X = bx; best2Y = by;
                    }
                }
            // Consistency check -> best match must be best match in backward direction
            x1 = bestX-r; if (x1 < 4) x1 = 4;
            y1 = bestY-r; if (y1 < 4) y1 = 4;
            x2 = bestX+r; if (x2 >= aXSize-4) x2 = aXSize-5;
            y2 = bestY+r; if (y2 >= aYSize-4) y2 = aYSize-5;
            int j = 0;
            for (int y = -4; y <= 4; y+=4)
                for (int x = -4; x <= 4; x+=4)
            ...
            for (int by = y1; by <= y2; by++)
                for (int bx = x1; bx <= x2; bx++) {
                    if (aEnergy1(bx,by) > 2.0f*aEnergy2(bestX,bestY) || aEnergy2(bestX,bestY) > 2.0f*aEnergy1(bx,by)) continue;
            ...
            if (fabs(bestXBack-ax) <= 1 && fabs(bestYBack-ay) <= 1) {
                ...
                aConfidence(ax,ay,0) = (best2Dist-bestDist)/(best2Dist+1e-6);
    ...
    inpaint(aMatching, aConfidence, t);
...
int main(int argc, char** args) {
    ...
    if (argc >= 3) {
        ...
        float sigma = 0.8f;
        if (argc >= 4) sigma = atof(args[3]);
        float alpha = 30;
        if (argc >= 5) alpha = atof(args[4]);
        float beta = 300;
        if (argc >= 6) beta = atof(args[5]);
        float gamma = 5;
        if (argc >= 7) gamma = atof(args[6]);

        int t;
        std::cout << "Please input t:\n" << std::endl;
        std::cin>>t;
        CTensor<float> aResult;
        runFlow(aImage1,aImage2,aResult,sigma,alpha,beta,gamma, t);
    ...
    else std::cout << "Usage: ldof image1.ppm image2.ppm" << std::endl;
    return 0;
}
