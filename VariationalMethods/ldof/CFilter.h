...
void boxFilterX(const CMatrix<T>& aMatrix, CMatrix<T>& aResult, int aWidth) {
  ...
  for (int y = 0; y < aMatrix.ySize(); y++) {
    ...
    T aSum = 0.0f;
    for (int x = 0; x <= aRight; x++) {
      aSum += aMatrix.data()[aOffset+x]; aNum++;
    }
    // Shift
    for (int x = 0; x < aMatrix.xSize(); x++) {
      aResult.data()[aOffset+x] = aSum*invWidth[aNum];
      if (x-halfWidth >= 0) {
        aSum -= aMatrix.data()[aOffset+x-halfWidth]; aNum--;
      }
      if (x+halfWidth+1 < aMatrix.xSize()) {
        aSum += aMatrix.data()[aOffset+x+halfWidth+1]; aNum++;
      }
    }
  }
  delete[] invWidth;
...
void boxFilterY(const CMatrix<T>& aMatrix, CMatrix<T>& aResult, int aWidth) {
  ...
  if (aBottom >= aMatrix.ySize()) aBottom = aMatrix.ySize()-1;
  for (int x = 0; x < aMatrix.xSize(); x++) {
    // Initialize
    int aNum = 0;
    T aSum = 0.0f;
    for (int y = 0; y <= aBottom; y++) {
      aSum += aMatrix(x,y); aNum++;
    }
    // Shift
    for (int y = 0; y < aMatrix.ySize(); y++) {
      aResult(x,y) = aSum*invWidth[aNum];
      if (y-halfWidth >= 0) {
        aSum -= aMatrix(x,y-halfWidth); aNum--;
      }
      if (y+halfWidth+1 < aMatrix.ySize()) {
        aSum += aMatrix(x,y+halfWidth+1); aNum++;
      }
    }
  }
  delete[] invWidth;
