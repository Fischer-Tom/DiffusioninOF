...
void COpticFlow::nonlinearIteration(CTensor<float>& aFirst, CTensor<float>& aSecond, CMatrix<float>& aConfidence,
  ...
  for (int i = 0; i < aSize; i++) {
    ...
    *(pST0+i) *= aConf; *(pST1+i) *= aConf; *(pST2+i) *= aConf; *(pST3+i) *= aConf; *(pST4+i) *= aConf; *(pST5+i) *= aConf;
    *(pST20+i) *= aConf; *(pST21+i) *= aConf; *(pST22+i) *= aConf; *(pST23+i) *= aConf; *(pST24+i) *= aConf; *(pST25+i) *= aConf;
  }
  // Precompute Laplacian of coarse U and V
  CMatrix<float> U1(aXSize,aYSize);  CMatrix<float> V1(aXSize,aYSize);
  CMatrix<float> U2(aXSize,aYSize);  CMatrix<float> V2(aXSize,aYSize);
  CMatrix<float> U3(aXSize,aYSize);  CMatrix<float> V3(aXSize,aYSize);
  CMatrix<float> U4(aXSize,aYSize);  CMatrix<float> V4(aXSize,aYSize);
  int i = 0;
...
void COpticFlow::warpingGeometric(CTensor<float> aFirst, CTensor<float> aSecond, CTensor<float>& aFlow,
  ...
  if (aFirst.xSize() < aMinSize) aMinSize = aFirst.xSize();
  int aMaxScale = (int)(log(15/aMinSize)/log(aEta));
  if (aMaxScale < 0) aMaxScale = 0;
  ...
  for (int s = aMaxScale; s >= -5; s--) {
    ...
    if (aXSize == aCoarseXSize && aYSize == aCoarseYSize) continue;
    int aSize = aXSize*aYSize;
    ...
    if (aScale == aMaxScale) aWarped = aSecondCoarse;
    else {
      int i = 0;
      for (int y = 0; y < aYSize; y++)
        for (int x = 0; x < aXSize; x++,i++) {
          ...
          int x1 = (int)(ax); int x2 = x1+1;
          int y1 = (int)(ay); int y2 = y1+1;
          ...
          else for (int k = 0; k < aWarped.zSize(); k++) {
    ...
    // Compute optic flow
    if (s == -1) aSORIterations *= 2;
    if (s < 0) aGeometricConfidenceCoarse = 0.0;
    if (aEdgeMap == 0) nonlinearIteration(aFirstCoarse,aWarped,aConfidence,aGeometricUCoarse,aGeometricVCoarse,aGeometricConfidenceCoarse,aFlow,0,aAlpha,aBeta,aGamma*pow(aEta,aScale),aEdgeWeight,aFixedpointIterations,aSORIterations,aOmega);
    else nonlinearIteration(aFirstCoarse,aWarped,aConfidence,aGeometricUCoarse,aGeometricVCoarse,aGeometricConfidenceCoarse,aFlow,&aEdgeMapCoarse,aAlpha,aBeta,aGamma*pow(aEta,aScale),aEdgeWeight,aFixedpointIterations,aSORIterations,aOmega);
  }
}
...
void COpticFlow::cartesianToRGB (float x, float y, float& R, float& G, float& B) {
  ...
  float phi;
  if (x == 0.0)
    if (y >= 0.0) phi = 0.5 * NMath::Pi;
    else phi = 1.5 * NMath::Pi;
  else if (x > 0.0)
    if (y >= 0.0) phi = atan (y/x);
    else phi = 2.0 * NMath::Pi + atan (y/x);
  else phi = NMath::Pi + atan (y/x);
...
void COpticFlow::warp(const CTensor<float>& aImage, CTensor<float>& aWarped, CMatrix<bool>& aOutside, CTensor<float>& aFlow) {
  ...
  for (int y = 0; y < aYSize; y++)
    for (int x = 0; x < aXSize; x++) {
      ...
      int y1 = (int)(ay); int y2 = y1+1;
      float alphaX = ax-x1; float alphaY = ay-y1;
      if (x1 < 0 || x2 >= aXSize || y1 < 0 || y2 >= aYSize) aOutside(x,y) = true;
      else for (int k = 0; k < aImage.zSize(); k++) {
...
void COpticFlow::writeMiddlebury(CTensor<float>& aFlow, const char* aFilename) {
  ...
  if (dot == NULL || strcmp(dot, ".flo") != 0) std::cout << aFilename << " should have extension '.flo'" << std::endl;
  FILE *stream = fopen(aFilename, "wb");
  if (stream == 0) std::cout << "Could not open " << aFilename << std::endl;
  // write the header
  fprintf(stream,"PIEH");
...
float COpticFlow::angularError(CTensor<float>& aFlow, CTensor<float>& aCorrect) {
  ...
  for (int y = 0; y < aFlow.ySize(); y++)
    for (int x = 0; x < aFlow.xSize(); x++) {
      if (fabs(aCorrect(x,y,0)) > 1e9 || fabs(aCorrect(x,y,1)) > 1e9) continue;
      float uc = aCorrect(x,y,0); float vc = aCorrect(x,y,1);
      float ue = aFlow(x,y,0);    float ve = aFlow(x,y,1);
